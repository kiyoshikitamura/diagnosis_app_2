                ...
                <div className="bg-white rounded-[2rem] shadow-sm p-8 space-y-6 border border-slate-50">
                    ...
                    <div className="space-y-4 pt-6 border-t border-slate-50">
                        <div className="grid grid-cols-3 gap-2">
                            <button onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(fullShareUrl)}`)} className="py-4 bg-slate-900 text-white rounded-2xl font-bold text-[10px] active:scale-95 transition-transform">𝕏で送る</button>
                            <button onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(fullShareUrl)}`, '_blank', 'width=600,height=400')} className="py-4 bg-[#1877F2] text-white rounded-2xl font-bold text-[10px] active:scale-95 transition-transform">Facebook</button>
                            <button onClick={() => {
...
export default function ResultClient() {
    return (
        <Suspense fallback={<div className="flex items-center justify-center min-h-screen bg-slate-50 text-slate-400 text-sm font-bold uppercase tracking-widest">Analyzing Mind...</div>}>
            <ResultContent />
        </Suspense>
    );
}